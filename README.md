...
No analytics, no Awin JS, no Google Fonts CDN. IBM Plex Sans and Source Serif 4 are self-hosted (SIL OFL) in `/fonts`. Drawings live in `/img`. Partner links only when a program has accepted; they belong on `balkonkraftwerk-kaufen.html` and must be labelled **Werbung**.
